...
const executePath_1 = require("./executePath");
const executeWhere_1 = require("./executeWhere");
const executeWhy_1 = require("./executeWhy");
const actionRegistry_1 = require("./actionRegistry");
function executeQuery(graph, query, bindings, actions) {
    switch (query.type) {
        case "MatchExpr":
            return executeMatch(graph, query, bindings);
        case "PathExpr":
            return (0, executePath_1.executePath)(graph, query);
        case "WhyExpr":
            return (0, executeWhy_1.executeWhy)(graph, query);
        case "HowExpr":
...
function executeWhereExpr(graph, query, bindings) {
    return (0, executeWhere_1.executeWhereQuery)(graph, query.expression, bindings);
}
function executeHow(query, bindings, actions) {
    ...
    const action = (0, actionRegistry_1.getAction)(actions, firstStep.binding);
    if (!action) {
...
function resolveHowTarget(query, bindings, actions) {
    ...
    return (0, evaluateNodeCapture_1.evaluateCapturedShape)(query.target, bindings, actions);
}
function executeMatch(graph, query, runtimeBindings) {
    const base = {
        kind: "MatchResultSet",
    ...
    if (query.where) {
        return (0, executeWhere_1.executeWhere)(graph, base, query.where);
    }
    return base;
}
function matchPatternBlock(graph, patterns, runtimeBindings) {
    ...
    for (const pattern of patterns) {
        ...
        for (const partial of partials) {
            for (const edge of graph.edges) {
                const matched = tryMatchRelationPattern(pattern, edge, partial.bindings, runtimeBindings);
                if (!matched)
                    continue;
                nextPartials.push({
                    bindings: matched,
...
function tryMatchRelationPattern(pattern, edge, existingBindings, runtimeBindings) {
    const leftBindings = tryMatchAtom(pattern.left, edge.subject, existingBindings, runtimeBindings);
    if (!leftBindings)
        return null;
    const relationBindings = tryMatchAtom(pattern.relation, edge.relation, leftBindings, runtimeBindings);
    if (!relationBindings)
        return null;
    const rightBindings = tryMatchAtom(pattern.right, edge.object, relationBindings, runtimeBindings);
    if (!rightBindings)
        return null;
    return rightBindings;
}
function tryMatchAtom(atom, candidate, bindings, runtimeBindings) {
    switch (atom.type) {
        case "Wildcard":
            return bindings;
        case "Identifier":
        ...
        default: {
            const _exhaustive = atom;
            throw new Error(`Unsupported pattern atom: ${JSON.stringify(_exhaustive)}`);
        }
    }
}
function matchIdentifierAtom(atom, candidate, bindings, runtimeBindings) {
...
function regexMatches(atom, candidate) {
...
function dedupePartials(partials) {
    const seen = new Set();
    const out = [];
    for (const partial of partials) {
...
function objectPropertyValueToDebug(value) {
    switch (value.type) {
        case "StringLiteral":
            return value.value;
        case "NumberLiteral":
...
function printTraversalValue(value) {
    switch (value.type) {
        case "Identifier":
            return value.name;
        case "StringLiteral":

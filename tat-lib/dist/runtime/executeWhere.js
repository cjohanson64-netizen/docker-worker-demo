...
exports.executeWhere = executeWhere;
exports.executeWhereQuery = executeWhereQuery;
function executeWhere(graph, source, condition) {
    const items = source.items.filter((item) => evaluateBooleanExpr(condition, graph, item.bindings));
    return {
...
function executeWhereQuery(graph, condition, bindings) {
...
function evaluateBooleanExpr(expr, graph, bindings) {
    switch (expr.type) {
...
function evaluateWhereTargetExpr(expr, context, item, bindings) {
    switch (expr.type) {
        case "BinaryBooleanExpr":
...
function evaluateBinaryBoolean(expr, graph, bindings) {
    if (expr.operator === "&&") {
        return (evaluateBooleanExpr(expr.left, graph, bindings) &&
...
function evaluateWhereBinaryBoolean(expr, context, item, bindings) {
    if (expr.operator === "&&") {
        return (evaluateWhereTargetExpr(expr.left, context, item, bindings) &&
...
function evaluateComparison(expr, graph, bindings) {
...
function evaluateWhereComparison(expr, context, item, bindings) {
...
function resolveBooleanValue(value, graph, bindings) {
    switch (value.type) {
        case "Identifier":
...
function resolveWhereBooleanValue(value, context, item, bindings) {
    switch (value.type) {
        case "Identifier":
...
function resolveIdentifier(name, bindings) {
    if (name in bindings) {
        return bindings[name];
    }
    return name;
}
function resolveWhereIdentifier(name, context, item, bindings) {
    if (name === "node") {
        return context === "node" ? whereNodeToValue(item) : null;
    }
    if (name === "edge") {
        return context === "edge" ? whereEdgeToValue(item) : null;
    }
...
function resolvePropertyAccess(access, graph, bindings) {
...
function resolveWherePropertyAccess(access, context, item, bindings) {
    const objectName = access.object.name;
    const path = access.chain.map((part) => part.name);
    if (objectName === "node") {
        if (context !== "node")
            return null;
        return dig(whereNodeToValue(item), path);
    }
    if (objectName === "edge") {
        if (context !== "edge")
            return null;
        return dig(whereEdgeToValue(item), path);
    }
...
function whereNodeToValue(node) {
    return {
        id: node.id,
...
function whereEdgeToValue(edge) {
    return {
        id: edge.id,
        from: edge.subject,
        to: edge.object,
...
function inferWhereReferenceKind(expr) {
    switch (expr.type) {
        case "BinaryBooleanExpr":
            return mergeWhereReferenceKinds(inferWhereReferenceKind(expr.left), inferWhereReferenceKind(expr.right));
        case "UnaryBooleanExpr":
            return inferWhereReferenceKind(expr.argument);
        case "GroupedBooleanExpr":
            return inferWhereReferenceKind(expr.expression);
        case "ComparisonExpr":
            return mergeWhereReferenceKinds(inferWhereValueReferenceKind(expr.left), inferWhereValueReferenceKind(expr.right));
        case "Identifier":
            if (expr.name === "node")
                return "node";
            if (expr.name === "edge")
            ...
            return "unknown";
        case "PropertyAccess":
            if (expr.object.name === "node")
                return "node";
            if (expr.object.name === "edge")
                return "edge";
            return "unknown";
        default:
            return "unknown";
...
function inferWhereValueReferenceKind(value) {
    switch (value.type) {
        case "Identifier":
            if (value.name === "node")
                return "node";
            ...
            return "unknown";
        case "PropertyAccess":
            if (value.object.name === "node")
                return "node";
            if (value.object.name === "edge")
                return "edge";
            return "unknown";
        default:
            return "unknown";
...
function mergeWhereReferenceKinds(left, right) {
...
function printBooleanExpr(expr) {
    switch (expr.type) {
        case "BinaryBooleanExpr":
